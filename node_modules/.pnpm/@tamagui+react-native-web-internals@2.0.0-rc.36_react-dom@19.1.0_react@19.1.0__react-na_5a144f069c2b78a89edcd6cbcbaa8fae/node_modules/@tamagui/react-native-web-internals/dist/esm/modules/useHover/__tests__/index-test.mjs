  ...
  beforeEach(() => {
    ...
    test("contains the hover gesture", () => {
      ...
      const target = createEventTarget(ref.current),
        child = createEventTarget(childRef.current);
      act(() => {
        target.pointerover(), target.pointerout(), child.pointerover();
      }), expect(onHoverEnd).toBeCalled(), act(() => {
        child.pointerout();
    ...
    test("is called for mouse pointers", () => {
      ...
      act(() => {
        target.pointerover();
      }), expect(onHoverChange).toBeCalledTimes(1), expect(onHoverChange).toBeCalledWith(!0), act(() => {
        target.pointerout();
    ...
    test("callbacks are called each time", () => {
      ...
      }), expect(onHoverStart).toBeCalledTimes(1), expect(onHoverUpdate).toBeCalledTimes(1), expect(onHoverEnd).toBeCalledTimes(1), expect(onHoverChange).toBeCalledTimes(2), act(() => {
        target.pointerover(), target.pointerhover({
          x: 1,
          y: 1
        }), target.pointerout();
