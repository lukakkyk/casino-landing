...
import { clearTimeoutIfSet, contains, getDocument, isElement, isMouseLikePointerType } from "./utils.mjs";
...
function useHover(context, props = {}) {
  ...
  const closeWithDelay = useEvent((event, runElseBranch = !0, reason = "hover") => {
      const closeDelay = getDelay(delayRef.current, "close", pointerTypeRef.current);
      closeDelay && !handlerRef.current ? (clearTimeoutIfSet(timeoutRef), timeoutRef.current = window.setTimeout(() => stableOnOpenChange(!1, event, reason), closeDelay)) : runElseBranch && (clearTimeoutIfSet(timeoutRef), stableOnOpenChange(!1, event, reason));
    }),
    ...
    isClickLikeOpenEvent = useEvent(() => dataRef.current.openEvent ? ["click", "mousedown"].includes(dataRef.current.openEvent.type) : !1);
  React.useEffect(() => {
    if (!enabled) return;
    function onReferenceMouseEnter(event) {
      if (clearTimeoutIfSet(timeoutRef), blockMouseMoveRef.current = !1, mouseOnly && !isMouseLikePointerType(pointerTypeRef.current) || restMsRef.current > 0 && !getDelay(delayRef.current, "open")) return;
  ...
  const reference = React.useMemo(() => {
    ...
    return {
      ...
      onMouseMove(event) {
        ...
        mouseOnly && !isMouseLikePointerType(pointerTypeRef.current) || open || restMsRef.current === 0 || restTimeoutPendingRef.current && event.movementX ** 2 + event.movementY ** 2 < 2 || (clearTimeoutIfSet(restTimeoutRef), pointerTypeRef.current === "touch" ? handleMouseMove() : (restTimeoutPendingRef.current = !0, restTimeoutRef.current = window.setTimeout(handleMouseMove, restMsRef.current)));
      }
    };
  }, [mouseOnly, open]);
