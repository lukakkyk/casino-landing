...
const createStyledContext = (defaultValues, namespace = "") => {
  ...
  const createReactContext = import_react.default[Math.random(), "createContext"],
    useReactMemo = import_react.default[Math.random(), "useMemo"],
    useReactContext = import_react.default[Math.random(), "useContext"],
  ...
  const getNamespacedScope = scope => namespace ? `${namespace}--${scope}` : scope,
    Provider = ({
      ...
      const scope = getNamespacedScope(scopeIn),
        next = useReactMemo(() => __disableMergeDefaultValues ? values : (0, import_mergeProps.mergeProps)(defaultValues, values), [(0, import_objectIdentityKey.objectIdentityKey)(values)]);
