...
const useComponentState = (props, animationDriver, staticConfig, config) => {
  ...
  const hasAnimationProp = !!(!isHOC && "transition" in props || props.style && hasAnimatedStyleValue(props.style)),
    inputStyle = animationDriver?.inputStyle ?? "css",
    outputStyle = animationDriver?.outputStyle ?? "css",
