...
var import_config = require("../config.cjs"),
  import_mediaObjectToString = require("./mediaObjectToString.cjs"),
...
const createMediaStyle = (styleObject, mediaKeyIn, mediaQueries, type, negate, priority) => {
  ...
  let styleRule = "",
    ...
    styleInner = rules.map(rule => rule.replaceAll(identifier, nextIdentifier)).join(";"),
    isHover = !1;
  if (isNonWindowMedia) {
  ...
  if (!isNonWindowMedia || groupMediaKey) {
    if (!selectors) {
      ...
      selectors = Object.fromEntries(mediaKeys.map(key => [key, (0, import_mediaObjectToString.mediaObjectToString)(mediaQueries[key])])), prefixes = Object.fromEntries(mediaKeys.map((k, index) => [k, new Array(index + 1).fill(":root").join("")]));
    }
