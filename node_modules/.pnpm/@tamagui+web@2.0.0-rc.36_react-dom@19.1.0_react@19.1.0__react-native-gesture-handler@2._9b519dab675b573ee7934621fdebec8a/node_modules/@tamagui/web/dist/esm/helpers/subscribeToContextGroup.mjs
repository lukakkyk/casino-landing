  ...
  createGroupListener = (name, {
    ...
    const dispose = parent.subscribe(({
      ...
      setStateShallow(prev => {
        ...
        if (pseudo && pseudoGroups?.has(name)) group.pseudo ||= {}, mergeIfNotShallowEqual(group.pseudo, pseudo) !== group.pseudo && (Object.assign(group.pseudo, pseudo), didChange = !0);else if (layout && mediaGroups) {
          group.media ||= {};
          const mediaState = getMediaState(mediaGroups, layout),
            next = mergeIfNotShallowEqual(group.media, mediaState);
