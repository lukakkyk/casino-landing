...
const createNativeMenu = MenuType => {
    ...
    return import_react.default.Children.forEach(children, child => {
      ...
      if (componentType === "CheckboxItem") {
        const {
          ...
          cleanChildren = import_react.default.Children.map(cbChildren, c => import_react.default.isValidElement(c) && (c.type?.displayName || "").includes("ItemIndicator") ? null : c);
  ...
  function lazyZeego(name, displayName) {
    const Comp = props => {
      const z = resolve();
      return z ? import_react.default.createElement(z.menu[name], props) : null;
    };
