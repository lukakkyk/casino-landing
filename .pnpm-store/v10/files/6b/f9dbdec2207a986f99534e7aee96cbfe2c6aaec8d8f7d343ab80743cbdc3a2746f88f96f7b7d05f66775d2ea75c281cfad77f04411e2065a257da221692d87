...
import { StyleObjectIdentifier, StyleObjectProperty, StyleObjectPseudo, StyleObjectRules, nonAnimatableStyleProps, stylePropsText, stylePropsTransform, tokenCategories, validPseudoKeys, validStyles as validStylesView } from "@tamagui/helpers";
import React from "react";
import { getConfig } from "../config.mjs";
import { isDevTools } from "../constants/isDevTools.mjs";
import { getMediaImportanceIfMoreImportant, getMediaKey, mediaKeyMatch } from "../hooks/useMedia.mjs";
...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, groupContext, elementType, startedUnhydrated, debug, animationDriver) => {
  ...
  for (const keyOg in props) {
    ...
    if (!noSkip && keyInit in skipProps && !(keyInit === "transition" && typeof valInit == "string" && !driver?.animations?.[valInit])) {
        ...
        if (!shouldDoClasses || isExit || isEnter) {
          ...
          for (const pkey in pseudoStyleObject) {
            ...
            if (isDisabled) applyDefaultStyle(pkey, styleState);else {
              ...
              shouldMerge && (process.env.IS_STATIC === "is_static" && (pseudos ||= {}, pseudos[key] ||= {}, pseudos[key][pkey] = val2), mergeStyle(styleState, pkey, val2, importance, !1, pseudoOriginalValues?.[pkey])), process.env.NODE_ENV === "development" && debug === "verbose" && log("    subKey", pkey, shouldMerge, {
                importance,
                curImportance,
                pkey,
                val: val2
        ...
        if (mediaStylesSeen += 1, shouldDoClasses) {
          ...
          for (const style of mediaStyles) {
            ...
            const subKey = isSubStyle ? style[2] : "",
              fullKey = `${style[StyleObjectProperty]}${subKey}${PROP_SPLIT}${mediaKeyShort}${style[StyleObjectPseudo] || ""}`;
          ...
          if (isThemeMedia) {
            ...
            if (groupPseudoKey) {
              ...
              const componentGroupPseudoState = (componentGroupState ||
                ...
                priority2 = pseudoPriorities[groupPseudoKey];
    ...
    if (!styleState.style.$$css) {
      ...
      for (const atomicStyle of atomic) {
        const [key, value, identifier] = atomicStyle,
          ...
          nonAnimatedTransitionOnly = !isAnimatedAndTransitionOnly && !styleProps.isAnimated && isClient && driver?.outputStyle === "css" && props.animateOnly?.includes(key);
  ...
  if (!styleProps.noMergeStyle && styleState.style && !shouldDoClasses && styleProps.isAnimated && !driver?.isReactNative && !styleState.style.$$css) {
    ...
    if (hasProps) {
      ...
      for (const atomicStyle of atomic) addStyleToInsertRules(rulesToInsert, atomicStyle), classNames[atomicStyle[StyleObjectProperty]] = atomicStyle[StyleObjectIdentifier];
    }
  }
...
function mergeStyle(styleState, key, val, importance, disableNormalize = !1, originalVal) {
  ...
  if (key in stylePropsTransform) styleState.flatTransforms ||= {}, usedKeys[key] = importance, styleState.flatTransforms[key] = val;else {
