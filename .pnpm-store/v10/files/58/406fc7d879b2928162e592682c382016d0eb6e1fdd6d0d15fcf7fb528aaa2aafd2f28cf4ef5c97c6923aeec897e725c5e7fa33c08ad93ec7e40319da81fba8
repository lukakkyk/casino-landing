...
function flattenChildrenKeyless(children) {
  ...
  return childrenArray.reduce(function (flatChildren, child) {
    return child.type === import_react.default.Fragment ? flatChildren.concat(flattenChildren(child.props.children)) : (flatChildren.push(child), flatChildren);
  }, []);
}
function flattenChildren(children, componentNamesToIgnore) {
  ...
  return import_react.Children.toArray(children).flatMap(function (elem) {
    return /* @__PURE__ */(0, import_react.isValidElement)(elem) && typeof elem?.type == "function" && componentNamesToIgnore?.some(function (skipComponentName) {
      return elem.type.displayName.includes(skipComponentName);
    }) ? elem.props.children : elem;
  }).reduce(function (acc, node, nodeIndex) {
    return node.type === import_react.default.Fragment ? acc.push.apply(acc, flattenChildren(node.props.children, componentNamesToIgnore, depth + 1, keys.concat(node.key || nodeIndex))) : /* @__PURE__ */(0, import_react.isValidElement)(node) ? acc.push(/* @__PURE__ */(0, import_react.cloneElement)(node, {
...
var pickChildren = function (_children, targetChild, componentNamesToIgnore) {
    var children = flattenChildren(_children, componentNamesToIgnore),
      target = [],
      withoutTargetChildren = import_react.default.Children.map(children, function (item) {
        return /* @__PURE__ */(0, import_react.isValidElement)(item) && isInstanceOfComponent(item, targetChild) ? (target.push(/* @__PURE__ */(0, import_react.cloneElement)(item)), null) : item;
      }),
