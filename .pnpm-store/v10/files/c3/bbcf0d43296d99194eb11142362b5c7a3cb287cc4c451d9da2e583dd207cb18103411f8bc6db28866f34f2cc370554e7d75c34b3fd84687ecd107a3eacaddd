...
const ToastRoot = React.forwardRef(function (props, _ref) {
    ...
    React.useEffect(() => {
    ...
    }, [notificationOptions]), React.useEffect(() => ToastState.subscribe(toast => {
      if (toast.dismiss) {
      ...
      native && dispatchNativeToast(toast, {
        duration,
  ...
  ToastViewport = ToastViewportFrame.styleable(function (props, ref) {
    const {
      ...
      offsetStyles = React.useMemo(() => {
        ...
        return yPosition === "top" ? styles.top = topOffset : styles.bottom = bottomOffset, isWeb ? xPosition === "left" ? styles.left = offsetObj.left ?? defaultOffset : xPosition === "right" ? styles.right = offsetObj.right ?? defaultOffset : (styles.left = "50%", styles.transform = "translateX(-50%)") : (styles.left = offsetObj.left ?? defaultOffset, styles.right = offsetObj.right ?? defaultOffset), styles;
    ...
    return portalToRoot ? /* @__PURE__ */jsx(Portal, {
      zIndex: portalZIndex,
      children: content
    }) : content;
  });
...
function DragWrapper({
  ...
  if (gesture) {
    ...
    if (GestureDetector) return /* @__PURE__ */jsx(GestureDetector, {
      gesture,
...
const ToastItemInner = ToastItemFrame.styleable(function (props, ref) {
    ...
    const {
      ...
      handleLayout = React.useCallback(event => {
        if (!isWeb || removed || !ctx.expanded && index !== 0) return;
        const {
          height
    ...
    if (isWeb) for (const t of ctx.toasts) {
