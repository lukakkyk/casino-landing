...
function createSheet(root, id = defaultId) {
  ...
  function createSheet2() {
    if (!sheet) if (canUseDOM) {
      const rootNode = root != null ? root.getRootNode() : document;
      if (sheets.length === 0) sheet = createOrderedCSSStyleSheet(createCSSStyleSheet(id)), initialRules.forEach(rule => {
        sheet.insert(rule, 0);
    ...
    } else sheets.length === 0 ? (sheet = createOrderedCSSStyleSheet(createCSSStyleSheet(id)), initialRules.forEach(rule => {
      sheet.insert(rule, 0);
